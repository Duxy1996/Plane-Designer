...
class EnginesControllerTest < ActionDispatch::IntegrationTest
  ...
  test "should create engine" do
    assert_difference('Engine.count') do
      post engines_url, params: { engine: { fuel_consumption: @engine.fuel_consumption, name: @engine.name, power: @engine.power } }
    end
  ...
  test "should update engine" do
    patch engine_url(@engine), params: { engine: { fuel_consumption: @engine.fuel_consumption, name: @engine.name, power: @engine.power } }
